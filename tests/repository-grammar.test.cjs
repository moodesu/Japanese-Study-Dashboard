...
const css=fs.readFileSync(path.join(root,'styles.css'),'utf8');assert.match(css,/\.repo-grammar-nav\{[^}]*display:inline-flex/);assert.match(css,/\.repo-grammar-nav\{[^}]*white-space:nowrap/);assert.match(css,/\.repo-grammar-nav\{[^}]*word-break:keep-all/);assert.match(css,/\.repo-import textarea\{[^}]*max-width:100%[^}]*overflow-x:auto/);assert.match(css,/\.repo-grammar-card>span[^}]*overflow-wrap:anywhere/);assert.match(css,/\.repo-grammar-card>strong\{[^}]*writing-mode:horizontal-tb/);assert.match(css,/@media\(max-width:760px\)\{\.repo-import textarea\{font-size:16px\}/,'mobile import textarea prevents iOS focus zoom');assert.match(css,/\.repo-page :is\([^}]+\)\{font-size:16px\}/,'other editable Repository controls use the same mobile font-size floor');
