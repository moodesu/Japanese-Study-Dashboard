...
const css=fs.readFileSync(path.join(root,'styles.css'),'utf8');assert.match(css,/\.repo-grammar-nav\{[^}]*display:inline-flex/);assert.match(css,/\.repo-grammar-nav\{[^}]*white-space:nowrap/);assert.match(css,/\.repo-grammar-nav\{[^}]*word-break:keep-all/);
