...
for(const heading of ['Overview','Formation','Usage and nuance','Combined forms','Clarifications','My examples','Reference examples','Related grammar'])assert.ok(html.includes(`>${heading}</h2>`),heading);
...
const standalone={routeStandalone:true};html=context.repositoryGrammarMarkup(standalone);assert.ok(!html.includes('<h2>In this sentence</h2>'));assert.ok(html.includes('>My examples</h2>'));assert.ok(html.includes('<mark class="repo-grammar-surface">てたら</mark>'));
state.grammarQuery='てたら';html=context.repositoryGrammarLibraryMarkup();assert.ok(html.includes('〜たら'));assert.ok(html.includes('class="repo-match-badge matched"'));assert.ok(html.includes('class="repo-match-badge related"'));assert.ok(html.includes('class="repo-match-kicker">Matched form'));assert.ok(html.includes('class="repo-match-kicker">Related match'));assert.equal((html.match(/class="repo-grammar-card"/g)||[]).length,2,'combined-form search also discovers its related canonical guide');assert.ok(!html.includes('data-repo-guide="v2"'),'variants are not cards');
...
state.grammarFilter='all';html=context.repositoryGrammarLibraryMarkup();assert.doesNotMatch(html,/Pending Guides|data-pending-guide|guide-generation prompt/,'retired pending-guide controls are absent');
assert.equal(context.repositorySavedGuide(tara).myExamples.length,1);
const css=fs.readFileSync(path.join(root,'styles.css'),'utf8');assert.match(css,/\.repo-grammar-nav\{[^}]*display:inline-flex/);assert.match(css,/\.repo-grammar-nav\{[^}]*white-space:nowrap/);assert.match(css,/\.repo-grammar-nav\{[^}]*word-break:keep-all/);assert.match(css,/\.repo-import textarea\{[^}]*max-width:100%[^}]*overflow-x:auto/);assert.match(css,/\.repo-grammar-card-meaning[^}]*overflow-wrap:anywhere/);assert.match(css,/\.repo-grammar-card-head>strong\{[^}]*writing-mode:horizontal-tb/);assert.match(css,/@media\(max-width:760px\)\{\.repo-import textarea\{font-size:16px\}/,'mobile import textarea prevents iOS focus zoom');assert.match(css,/\.repo-page :is\([^}]+\)\{font-size:16px\}/,'other editable Repository controls use the same mobile font-size floor');
