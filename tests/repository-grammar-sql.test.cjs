...
(async()=>{const db=new PGlite();try{
  ...
  const entry={...sentence,grammar_points:sentence.grammar_points.map(x=>x.canonical)};const payload=[{entry,grammar_points:sentence.grammar_points}];
  ...
  assert.equal(imported.links.length,3);const patterns=(await db.query('select pattern from public.japanese_grammar_guides order by pattern')).rows.map(x=>x.pattern);assert.ok(patterns.includes('〜たら'));assert.ok(!patterns.includes('〜てたら'));
  ...
  assert.equal(Number((await db.query('select count(*) from public.japanese_grammar_clarifications')).rows[0].count),1);assert.equal(Number((await db.query('select count(*) from public.japanese_repository')).rows[0].count),2);
  ...
  console.log('PASS: reset preserves sentences, canonical placeholders, no surface guide, guide upgrade, variants, links and clarification isolation.');
