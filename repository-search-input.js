...
/*
 * Phase 7B: retire the Pending Guides runtime UI.
 *
 * The canonical catalogue no longer supports implicit placeholder creation.
 * Phase 8 will remove the dead pending-specific branches from repository.js;
 * this transitional wrapper removes the obsolete controls immediately without
 * changing the accepted Grammar Library card/layout implementation.
 */
(()=>{
  if(typeof repositoryState==='undefined')return;

  const stripPendingUi=html=>{
    const root=document.createElement('div');
    root.innerHTML=String(html||'');

    // Pending is no longer a supported catalogue state, so don't expose a
    // filter/action surface for it even if stale markup exists in repository.js.
    root.querySelectorAll('[data-grammar-filter="pending"], .repo-pending-actions, .repo-pending-select, .repo-guide-pending, #repoViewPending, #repoCopySingleGuidePrompt')
      .forEach(node=>node.remove());

    root.querySelectorAll('.repo-grammar-card-wrap.is-pending')
      .forEach(node=>node.classList.remove('is-pending'));

    return root.innerHTML;
  };

  if(typeof repositoryGrammarLibraryMarkup==='function'){
    const baseLibraryMarkup=repositoryGrammarLibraryMarkup;
    window.repositoryGrammarLibraryMarkup=function(...args){
      if(repositoryState.grammarFilter==='pending'){
        repositoryState.grammarFilter='all';
      }
      repositoryState.pendingGuideIds?.clear?.();
      return stripPendingUi(baseLibraryMarkup.apply(this,args));
    };
  }

  if(typeof repositoryGrammarMarkup==='function'){
    const baseGrammarMarkup=repositoryGrammarMarkup;
    window.repositoryGrammarMarkup=function(...args){
      return stripPendingUi(baseGrammarMarkup.apply(this,args));
    };
  }
})();
