...
function resetRepositorySession(){
  ...
  repositoryState.grammarGuideId=null; repositoryState.grammarQuery=''; repositoryState.grammarFilter='all'; pendingRepositoryImport=null; repositoryImportSnapshot='';
}
...
function repositoryGrammarMarkup(entry){
  ...
  const body=guide?`
    <article class="repo-guide-article">
      <section class="repo-guide-section">${repositoryGuideHeading('Overview')}${guide.summary?`<p>${text(guide.summary)}</p>`:''}</section>
...
function repositoryGrammarLibraryMarkup(){
  ...
  const filter=repositoryState.grammarFilter==='encountered'?'encountered':'all';
  for(const guide of repositoryState.grammarGuides){
    ...
    const visible=filter!=='encountered'||encountered(guide);
  ...
  if(query){
    for(const variant of repositoryState.grammarVariants.filter(row=>row.related_grammar_id&&row.form.toLowerCase().includes(query))){
      ...
      const visible=guide&&(filter!=='encountered'||encountered(guide));
      if(visible){matches.push({guide,variant,relatedSource:source});matchedIds.add(guide.id);}
    }
  }
  const directCount=repositoryState.grammarGuides.filter(encountered).length;
  const filters=[['all',`All ${repositoryState.grammarGuides.length}`],['encountered',`Encountered in my sentences ${directCount}`]];
  const card=({guide,variant,relatedSource})=>{const match=repositoryGrammarMatchMarkup(variant,relatedSource);return `<article class="repo-grammar-card-wrap"><button type="button" class="repo-grammar-card" data-repo-guide="${esc(guide.id)}"><span class="repo-grammar-card-head"><strong lang="ja">${renderRepositoryFurigana(guide.pattern,repositoryPlainFromFurigana(guide.pattern))}</strong>${match.badge}</span><span class="repo-grammar-card-meaning">${esc(guide.meaning)}</span>${match.detail}</button></article>`;};
  return `<section class="repo-page"><div class="repo-page-head app-page-header"><div><button class="app-page-breadcrumb" id="repoGrammarLibraryBack">← Repository</button><div class="eyebrow">Canonical grammar</div><h1>Grammar Library</h1><p>Reusable grammar concepts from your saved Japanese.</p></div><div class="app-page-toolbar"><button class="smallbtn" id="repoImport">Import JSON</button></div></div><div class="repo-grammar-filters app-compact-stats" role="group" aria-label="Grammar guide filters">${filters.map(([value,label])=>`<button type="button" class="smallbtn ${filter===value?'primary':''}" data-grammar-filter="${value}">${label}</button>`).join('')}</div><section class="panel repo-grammar-search"><label><span>Search patterns, meanings or forms</span><input id="repoGrammarSearch" type="search" value="${esc(repositoryState.grammarQuery)}" placeholder="Try てたら, ちゃった or ている"></label></section><div class="repo-grammar-library-grid">${matches.length?matches.map(card).join(''):'<div class="empty panel">No canonical grammar guides match this search.</div>'}</div></section>`;
}
...
function bindRepositoryEvents(selected){
  ...
  document.querySelectorAll('[data-grammar-filter]').forEach(x=>x.onclick=()=>{repositoryState.grammarFilter=x.dataset.grammarFilter;renderRepository();});
...
function previewRepositoryImport(){
  ...
  try{
    ...
    if(Array.isArray(parsed)&&parsed.length&&parsed.every(item=>item?.entry_type==='grammar_guide')){
      ...
      const operations=guides.map(guide=>{const existing=repositoryState.grammarGuides.find(item=>repositoryGrammarKey(item.pattern)===repositoryGrammarKey(guide.canonical));return {guide,operation:!existing?'new':'update'};});
      pendingRepositoryImport={kind:'guides',payload:guides};
      const counts=type=>operations.filter(item=>item.operation===type).length;
      $('#repoImportPreview').innerHTML=`<strong>${guides.length} grammar guides</strong><span>${counts('new')} new guides · ${counts('update')} existing guides will be updated</span>${operations.map(item=>`<span lang="ja">${esc(item.guide.canonical)} · ${item.operation}</span>`).join('')}`;
    ...
    if(parsed?.entry_type==='grammar_guide'){
      ...
      const operation=!existing?'New guide':'Updates existing guide';
    ...
    const unresolved=new Set(plan.items.flatMap(item=>item.grammar_points).filter(point=>!repositoryState.grammarGuides.some(guide=>repositoryGrammarKey(guide.pattern)===repositoryGrammarKey(point.canonical))).map(point=>point.canonical));
    $('#repoImportPreview').innerHTML=`<strong>${sentenceSummary} · ${links} canonical grammar link${links===1?'':'s'}</strong>${unresolved.size?`<span>${unresolved.size} grammar label${unresolved.size===1?'':'s'} require live canonical resolution before import: ${[...unresolved].map(esc).join(', ')}</span>`:''}${plan.items.slice(0,5).map(item=>`<span lang="ja">${repositoryJapanese(item.entry)}</span>`).join('')}${plan.warnings.map(value=>`<p>${esc(value)}</p>`).join('')}`;
  ...
  try{
    ...
    if(pending.kind==='guides'){
      ...
      if(incomplete.length){
        ...
        throw new Error(`Batch import incomplete: ${completed} of ${pending.payload.length} guides completed. Incomplete: ${incomplete.map(guide=>guide.canonical).join(', ')}`);
      }
    }
