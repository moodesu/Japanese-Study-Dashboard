-- Retire source metadata from personal Repository sentences.
-- Columns remain for backwards compatibility, but sentence creation/import no longer uses them.

alter table public.japanese_repository
  alter column source_type set default '',
  alter column source_detail set default '';

update public.japanese_repository
set source_type='', source_detail=''
where coalesce(source_type,'')<>'' or coalesce(source_detail,'')<>'';
...
begin
  ...
  for item in select value from jsonb_array_elements(p_entries) loop
    ...
    select * into imported from jsonb_populate_record(null::public.japanese_repository,payload);
    if imported.japanese is null or trim(imported.japanese)=''
      or (coalesce(imported.japanese_furigana,'')<>'' and regexp_replace(imported.japanese_furigana,'\\[([^]|]+)\\|([^]]+)\\]','\\1','g')<>imported.japanese) then
      raise exception 'Invalid sentence or furigana.';
    end if;

    sentence=null;
    if coalesce(imported.entry_type,'sentence')='sentence' then
      select * into sentence from public.japanese_repository
        where user_id=caller and entry_type='sentence' and japanese=imported.japanese
        for update;
    end if;
    ...
    if sentence.id is null then
      insert into public.japanese_repository(
        ...
        book_id,register,error_types,notes
      ) values (
        caller,coalesce(imported.entry_type,'sentence'),coalesce(imported.status,'learning'),coalesce(imported.intent_english,''),
        coalesce(imported.original_japanese,''),coalesce(imported.original_japanese_furigana,''),imported.japanese,
        coalesce(imported.japanese_furigana,''),coalesce(imported.english,''),coalesce(imported.explanation,''),
        coalesce(imported.grammar_points,'{}'),coalesce(imported.tags,'{}'),imported.lesson_number,imported.book_id,
        coalesce(imported.register,'neutral'),coalesce(imported.error_types,'{}'),coalesce(imported.notes,'')
      ) returning * into sentence;
      operation='created'; created_count=created_count+1;
    else
      update public.japanese_repository set
      ...
      where id=sentence.id and user_id=caller returning * into sentence;
      delete from public.japanese_repository_grammar where repository_id=sentence.id and user_id=caller;
      operation='updated'; updated_count=updated_count+1;
    end if;

    canonicals='{}';
    for annotation in select value from jsonb_array_elements(item->'grammar_points') loop
      if jsonb_typeof(annotation) is distinct from 'object'
        ...
        or (annotation ? 'guide_slug' and jsonb_typeof(annotation->'guide_slug')<>'string') then
      ...
      if guide.id is null then
        raise exception 'Sentence import references missing canonical grammar: %. Resolve the label against the Grammar Library before importing.',annotation->>'canonical';
      end if;
      ...
      if guide.guide_status<>'complete' or guide.is_placeholder then
        raise exception 'Sentence import references incomplete canonical grammar: %. Complete the guide before importing the sentence.',guide.pattern;
      end if;

      insert into public.japanese_repository_grammar(repository_id,user_id,grammar_id,surface,note)
        values(sentence.id,caller,guide.id,annotation->>'surface',annotation->>'note')
        returning * into linkrow;
      if not(guide.pattern=any(canonicals)) then canonicals=array_append(canonicals,guide.pattern); end if;
      result_guides=result_guides||jsonb_build_array(to_jsonb(guide));
      result_links=result_links||jsonb_build_array(to_jsonb(linkrow));
    end loop;

    update public.japanese_repository set grammar_points=canonicals where id=sentence.id returning * into sentence;
    result_entries=result_entries||jsonb_build_array(to_jsonb(sentence));
    result_operations=result_operations||jsonb_build_array(jsonb_build_object('id',sentence.id,'operation',operation));
  end loop;

  return jsonb_build_object(
    'entries',result_entries,'guides',result_guides,'links',result_links,'operations',result_operations,
