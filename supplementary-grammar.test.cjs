...
(async()=>{
  ...
  assert.match(styles,/\.supplementary-resource-card \.secondary-action \{[^}]*align-self:flex-start[^}]*min-height:40px/,'The workbook uses a compact content-width secondary action');
  assert.match(app,/hubSectionMarkup\(\{id:'books'/,'The book library is a collapsible logical Hub section');
  assert.match(app,/hubSectionMarkup\(\{id:'study-tools'/,'Study tools are a collapsible logical Hub section');
  assert.match(app,/bindHubSectionToggles\(\$\('#mainContent'\)\)/,'Hub section controls are bound after each render');

  // Hub section state uses stable logical keys, is expanded by default and
  // survives both user-triggered and supplementary-data rerenders.
  const stored=new Map();
  let sectionRenders=0;
  const sectionContext={
    localStorage:{getItem:key=>stored.has(key)?stored.get(key):null,setItem:(key,value)=>stored.set(key,value)},
    esc:value=>String(value),
    renderLibrary:()=>{sectionRenders++;}
  };
  vm.createContext(sectionContext);
  const helperStart=app.indexOf("const HUB_SECTION_STORAGE_PREFIX='");
  const helperEnd=app.indexOf('\nfunction renderLibrary()',helperStart);
  vm.runInContext(app.slice(helperStart,helperEnd),sectionContext);
  const expanded=sectionContext.hubSectionMarkup({id:'study-tools',title:'Study tools & input',subtitle:'Supporting resources stay independent.',content:'<div>cards</div>'});
  assert.match(expanded,/aria-expanded="true"/);
  assert.doesNotMatch(expanded,/id="hub-section-study-tools-content" hidden/);
  const attributes=new Map([['aria-expanded','true']]);
  const toggle={dataset:{hubSectionToggle:'study-tools'},getAttribute:name=>attributes.get(name)};
  sectionContext.bindHubSectionToggles({querySelectorAll:()=>[toggle]});
  toggle.onclick();
  assert.equal(stored.get('learningHub.section.study-tools.collapsed'),'true');
  assert.equal(sectionRenders,1);
  const collapsedAfterClick=sectionContext.hubSectionMarkup({id:'study-tools',title:'Study tools & input',content:'<div>cards</div>'});
  assert.match(collapsedAfterClick,/aria-expanded="false"/);
  assert.match(collapsedAfterClick,/id="hub-section-study-tools-content" hidden/);
  const collapsedAfterAsyncLoad=sectionContext.hubSectionMarkup({id:'study-tools',title:'Study tools & input',content:'<div>workbook loaded</div>'});
  assert.match(collapsedAfterAsyncLoad,/aria-expanded="false"/,'The async supplementary rerender reads and preserves the stored collapsed state');
  sectionContext.setHubSectionCollapsed('study-tools',false);
  assert.match(sectionContext.hubSectionMarkup({id:'study-tools',title:'Study tools & input',content:'<div>cards</div>'}),/aria-expanded="true"/,'An explicit expand choice is restored');
